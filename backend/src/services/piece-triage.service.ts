//backend/src/services/piece-triage.service.ts
...
export class PieceTriageService {
    ...
    const [data, total] = await Promise.all([
      prisma.pieceTriage.findMany({
        ...
        include: {
          ...
          siteExpedition: true,
          fournisseur:   { select: { id: true, nom: true } },
          referencePiece:{ select: { photo1: true, photo2: true } },
        },
        orderBy: { dateTri: "desc" },
        skip,
        take: limit,
  ...
  static async create(dto: CreateTriageDto, agentTriId: string) {
    ...
    // Créer le Set s'il n'existe pas encore (clé naturelle = code 5 chiffres)
    if (dto.setId) {
      await prisma.set.upsert({
        ...
        create: {
          id:               dto.setId,
          siteExpeditionId: dto.siteExpeditionId ?? undefined,
        },
      });
    }

    // Auto-apprentissage SET
    if (dto.nitgSaisi && dto.refPieceCauseSaisie && dto.setId) {
      const res = await CorrespondanceSetService.upsertFromTriage({
        nitg:             dto.nitgSaisi,
        refPieceCause:    dto.refPieceCauseSaisie,
        typePiece:        dto.typePiece,
        setId:            dto.setId,
        projetVehicule:   dto.projetVehicule,
        indiceVehicule:   dto.indiceVehicule,
        projetMoteur:     dto.projetMoteur,
        indiceMoteur:     dto.indiceMoteur,
        projetBoite:      dto.projetBoite,
        indiceBoite:      dto.indiceBoite,
        siteExpeditionId: dto.siteExpeditionId,
        fournisseurId:    dto.fournisseurId,
        codeRef6Plus2:    dto.codeRef6Plus2,
        referencePieceId: dto.referencePieceId,
      }, agentTriId);
      correspondanceSetId = res.correspondance.id;
    }

    const triage = await prisma.pieceTriage.create({
      data: {
        ...dto,
        agentTriId,
        correspondanceSetId,
    ...
    await AuditService.log({
      ...
      entite:   "PieceTriage",
      entiteId: triage.id,
      details:  `${dto.typePiece} — ${dto.nitgSaisi}`,
      userId:   agentTriId,
    });
  ...
  static async update(id: string, dto: UpdateTriageDto) {
  ...
  static async caffuter(id: string, dto: CaffuterDto, userId: string) {
    ...
    const upd = await prisma.pieceTriage.update({
      where: { id },
      data: {
        caffute:          true,
        caffutageMotif:   dto.motif,
        caffuteParUserId: userId,
        caffuteAt:        new Date(),
        statutIC:         "CAFFUTE",
      },
      include: INC,
    ...
    await AuditService.log({
      action: "TRIAGE_CAFFUTER", entite: "PieceTriage", entiteId: id, details: dto.motif, userId,
    });
    return upd;
  }
