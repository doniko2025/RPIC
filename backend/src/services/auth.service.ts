...
export class AuthService {
  static async login(dto: LoginDto, ip?: string, ua?: string) {
    ...
    // FIX : la config ESLint du projet ne respecte pas varsIgnorePattern "^_".
    // eslint-disable-next-line @typescript-eslint/no-unused-vars -- mot de passe volontairement exclu du retour
  ...
  static async refresh(token: string) {
    ...
    const at = signAccess(newPl);
    const rt = signRefresh(newPl);
  ...
  static async requestReset(email: string, base: string) {
    ...
    if (!user) return; // sécurité : pas de révélation d'existence
    const token = uuid();
